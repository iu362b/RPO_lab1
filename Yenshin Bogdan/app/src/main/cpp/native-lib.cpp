...
auto android_logger = spdlog::android_logger_mt("android", "lab_ndk");
...
//extern "C" JNIEXPORT jstring JNICALL
extern "C" JNIEXPORT jstring JNICALL
Java_com_example_lab_MainActivity_stringFromJNI(JNIEnv* env, jobject /* this */){
std::string hello = "Hello from C++";
...
Java_com_example_lab_MainActivity_randomBytes(JNIEnv *env, jclass clazz, jint no) {
...
extern "C"
JNIEXPORT jbyteArray JNICALL
Java_com_example_lab_MainActivity_encrypt(JNIEnv *env, jclass clazz, jbyteArray key,
                                          jbyteArray data) {
...
extern "C"
JNIEXPORT jbyteArray JNICALL
Java_com_example_lab_MainActivity_decrypt(JNIEnv *env, jclass clazz, jbyteArray key,
                                          jbyteArray data) {
    jsize ksz = env->GetArrayLength(key);
    jsize dsz = env->GetArrayLength(data);
...
Java_com_example_lab_MainActivity_transaction(JNIEnv *xenv, jobject xthiz, jbyteArray xtrd) {
    ...
    std::thread t([thiz, trd] {
        ...
        while (ptc > 0) {
            ...
            if ((utf != nullptr) && (strcmp(utf, "9367") == 0))
                break;
            ptc--;
        }
